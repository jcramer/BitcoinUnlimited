...
extern "C" int Bin2Hex(unsigned char* val, int length, char* result, unsigned int resultLen)
{
    ...
    if (s.size() >= resultLen) return 0;  // need 1 more for /0
...
*/
extern "C" int SignTx(unsigned char* txData, int txbuflen, unsigned int inputIdx, int64_t inputAmount, unsigned char* prevoutScript, uint32_t priorScriptLen, uint32_t nHashType, unsigned char* keyData, unsigned char* result, unsigned int resultLen)
{
